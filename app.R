...
ui <- fluidPage(
    ...
    navbarPage(
      ...
      tabPanel("Contact", icon = icon("envelope"),
        h3("Any bugs/issues?"),
        h4(tags$ul(tags$li(a("Send me a message", href = "mailto:hello@bcfalk@gmail.com.com", target = "_blank")))),
        h3("Want to connect?"),
        h4(tags$ul(tags$li("Follow me on", a("GitHub", href = "https://github.com/BrendanFalk557", target = "_blank"))))
        )
      )
    )
######################################## Define server ##########################################
